 ...
 * ```
 */
type EndpointInitiateResult<Result> = Promise<{
    data: Result;
    error?: undefined;
} | {
    error: any;
...
/** Normal query hooks */
{
    [K in keyof T as T[K] extends {
        type: 'query';
    } ? `use${Capitalize<string & K>}Query` : never]: T[K] extends QueryDef<infer Arg, infer Result, any> ? QueryHookWithInitiate<Arg, Result> : never;
}
/** Lazy query hooks */
 & {
    [K in keyof T as T[K] extends {
        type: 'query';
    } ? `useLazy${Capitalize<string & K>}Query` : never]: T[K] extends QueryDef<infer Arg, infer Result, any> ? LazyQueryHookWithInitiate<Arg, Result> : never;
}
/** Mutation hooks */
 & {
    [K in keyof T as T[K] extends {
        type: 'mutation';
    } ? `use${Capitalize<string & K>}Mutation` : never]: T[K] extends MutationDef<infer Arg, infer Result, any> ? MutationHookWithInitiate<Arg, Result> : never;
}
